 ...
 *
 */
public final class AliCloudDDNS {

	/* Area ID */
	private static final String[] ACCESS_AREA_ID = { "cn-qingdao", "cn-beijing", "cn-shanghai", "cn-hangzhou"};
	/* AccessKey ID */
	...
	/* 主域名 */
	private static final String DOMAIN_NAME = "yuhaijun.com.cn";
	/* 主机记录 */
	private static final String[] RRKEY_WORD = { "*", "@", "www" };
	/* 记录类型 */
	private static final String[] RRKEY_TYPE = { "A", "CNAME", "MX", "AAAA", "TXT", "显性URL", "隐性URL", "NS", "SRV",
			"CAA", "PTR" };

	public static void main(String[] args) {
		// 设置鉴权参数，初始化客户端
		DefaultProfile profile = DefaultProfile.getProfile(ACCESS_AREA_ID[0], ACCESS_KEY_ID, ACCESS_KEY_SECRET);
		IAcsClient client = new DefaultAcsClient(profile);

		AliCloudDDNS ddns = new AliCloudDDNS();
		...
		// 主机记录
		describeDomainRecordsRequest.setRRKeyWord(RRKEY_WORD[0]);
		// 解析记录类型
		describeDomainRecordsRequest.setType(RRKEY_TYPE[0]);
		...
		if (domainRecords.size() != 0) {
			...
			if (!currentHostIP.equals(recordsValue)) {
				...
				// 主机记录
				updateDomainRecordRequest.setRR(RRKEY_WORD[0]);
				// 记录ID
				updateDomainRecordRequest.setRecordId(recordId);
				// 将主机记录值改为当前主机IP
				updateDomainRecordRequest.setValue(currentHostIP);
				// 解析记录类型
				updateDomainRecordRequest.setType(RRKEY_TYPE[0]);
